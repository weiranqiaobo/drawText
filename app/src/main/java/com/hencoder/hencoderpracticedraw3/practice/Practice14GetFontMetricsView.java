...
public class Practice14GetFontMetricsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float dash = Math.abs((fontMetrics.descent + fontMetrics.ascent) / 2);
        int middle = (top + bottom) / 2;
        // 这种居中算法的优点是，可以让不同的文字的 baseline 对齐
        canvas.drawText(texts[0], 100, middle + dash, paint2);
        canvas.drawText(texts[1], 200, middle + dash, paint2);
        canvas.drawText(texts[2], 300, middle + dash, paint2);
        canvas.drawText(texts[3], 400, middle + dash, paint2);
        canvas.drawText(texts[4], 500, middle + dash, paint2);
        canvas.drawText(texts[5], 600, middle + dash, paint2);
    }
}
